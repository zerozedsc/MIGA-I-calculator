export interface SmartSuggestion {
  ...
  naivePrice: number; // RM (2dp) - what user "expects" to pay for displayedWeight
  savingsAmount: number; // RM (2dp) - naivePrice - suggestedPrice
  savingsPercentage: number; // % (2dp)
  isBest: boolean;
}
...
const findOptimalPriceForWeight = (
  ...
  const naivePrice = targetWeight * standardPrice;
  const savingsAmount = naivePrice - optimalPrice;
  const savingsPercentage = naivePrice > 0 ? (savingsAmount / naivePrice) * 100 : 0;
  ...
  return {
    ...
    naivePrice: Number(naivePrice.toFixed(2)),
...
export const findSmartPriceSuggestions = (
  ...
  for (const targetWeight of targetWeights) {
    ...
    // Only include if meaningful savings (per reference spec)
    if (suggestion.savingsAmount > 0.05 && suggestion.savingsPercentage > 0.3) {
